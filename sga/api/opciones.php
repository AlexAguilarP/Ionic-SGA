...
switch ($opcion) {
	case '1':
		//obtener lista de asistencia de las actividade
		$id_lista = $_GET['id_lista'];
		$nuevoSql = "SELECT * FROM lista_actividades WHERE id_actividad = '$id_lista'";
		$ql = mysqli_query($con, $nuevoSql);
	...
	case '4':
		...
		if (strcmp($id_rol, $i2) === 0 || strcmp($id_rol, $i3) === 0 || strcmp($id_rol, $i4) === 0) {
			...
			*/
			$nuevoSql = "SELECT id_lista_docente FROM lista_docentes WHERE id_lista='$id_lista'AND id_docente='$id'";
			$ql = mysqli_query($con, $nuevoSql);
			while ($row = mysqli_fetch_array($ql)) {
				$id_lista_docente = $row['id_lista_docente'];
			}
			//$num = $nuevoSql->num_rows;
			if (empty($id_d)) {
			...
			if ($num == 0) {
				...
				$sql = "UPDATE `lista_estudiantes` SET `updated_at`='$fin' WHERE id_lista_estudiante='$id_lista_estudiante'";
				echo putSQL($sql);
			}
		}
		break;
